...
app.get('/', (req, res, next) => {
    posts.find({},{comments:showComments}).toArray((err, thePosts) => {
        if (err) {
            next(err);
        }
        res.render('posts', {
...
app.get('/showComments',(req,res,next)=>{
    ...
    posts.find({_id: new mongo.ObjectId(req.query.id)},{comments:1}).toArray((err,comments)=> {
        console.log(comments[0].comments);
        res.send(JSON.stringify(comments[0]));
        }
    );
});
